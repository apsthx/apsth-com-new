...
export default function SquareFeatureCard({ item }) {
  ...
  return (
    <div className="group relative aspect-[4/5] min-h-[180px] w-full rounded-md bg-[#45c4a0] text-white flex flex-col items-center justify-between py-3 px-2 sm:p-4 text-center overflow-hidden border border-white/10 transition-all duration-300 cursor-default shadow-sm hover:shadow-md">
      {/* Ribbon Badge */}
      {item.badge && (
      ...
      {item.sparkle && (
        <div className="absolute inset-0 pointer-events-none opacity-30">
          <i className="uil uil-star absolute top-2 right-2 text-[10px] " />
          <i className="uil uil-star absolute bottom-4 left-2 text-[8px] " />
        </div>
      )}

      {/* ส่วนบน: Icon Section */}
      <div className="mt-2 flex-shrink-0 ">{item.icon}</div>
