...
// Package.js (Server Component)
import React from "react";
import PackageTable from "./PackageTable"; // เปลี่ยนจาก PackageGroups เป็นตาราง
import { useTranslation } from "@/app/i18n";

export default async function Package({ lng }) {
  const { t } = await useTranslation(lng);

  // --- translations object คงไว้ตามเดิมของคุณทุกประการ ---
  const translations = {
    selectPlan: t("read_more", "ดูรายละเอียด"),
    addOnLabel: t("Add On"),
    billingCycles: {
    ...
    features: {
      branch_count: t("home:package.name.branches", "จำนวนสาขา"),
      user_count: t("home:package.name.user", "User"),
      file_uploads: t("home:package.name.file_uploads", "File uploads"),
      card_reader: t(
        "home:package.name.free_card_reader",
        "ฟรี เครื่องอ่านบัตร",
  ...
  return (
    ...
    >
      <div className="text-center mb-4">
        ...
        <p className="text-[0.8rem] text-[#343f52] opacity-70">
          ({t("home:package.description", "ราคา Package")})
        </p>
      </div>

      {/* เรียกใช้ Component ตาราง */}
      <PackageTable translations={translations} />
    </section>
  );
}
